...
## 2026-09-13 Innovation Atlas 全量主张复核

- 逐项复核195对象、160关系和172原证据，补26项材料，现为198来源；修复版本、证据范围和原文入口。
- 补正开发者确认的影响与系列发展，保留Dominion作者反例；19条编辑比较撤去因果箭头。
- 比较统一标为“机制比较”；阅读路线覆盖与因果证明分开验收。详见[核查说明](docs/devlog/2026-09-13-atlas-full-verification.md)。

## 2026-09-12 空间、输入与近期版本

- 再增49历史对象（含6机制观察与3桌面游戏）、40关系和45来源，目录195/160/172；补足三维格斗、平台、赛车、体感、移动与中文作品，并加入2025–2026版本实例。
- 七个比较入口保持同一catalog；修复初始深链接在手机上落到年代目录、选中地图节点不在视野的问题。
- [公开设计说明](docs/devlog/2026-09-12-how-gameplay-changes.md)；具体来源与失败记录见第三、第四批研究回执。
...
- 新增32个游戏/版本对象、6个机制观察与20条关系；目录达到134对象 / 106关系 / 113来源。补入RPG、模拟、在线世界、卡牌、格斗、竞速、音乐及中文作品，并覆盖到2024年。
- 新总览扩为13个阅读区域；精确时间轴从数据推导末端刻度，避免2020年后对象落出画布。来源语种支持简体与繁体中文。
- 明确早期体验、资料片和重制版的日期语境；保留UO与Minecraft的无向结构比较，不添加未证实的Slay the Spire → Balatro或Minecraft原版 → Factorio直接影响边。

## 2026-09-12 · 创新历史总览

- 修复精确年份网络中文献跳转与位置恢复受全局平滑滚动干扰的问题，返回时保留原地图位置和键盘焦点。

- `/atlas/` 重做为年代 × 九条阅读脉络的总览。作品与机制共处一图，新增三个比较阅读入口、搜索、相邻关系与浮动证据面板；保留全图位置、浏览器返回和无脚本年代目录。
- 原精确年份网络移至 `/atlas/network/`，仍使用同一个 catalog。修复内容扩展后场景高度未覆盖作品下边界的问题。
- 依据两家博物馆年表补入 12 个历史对象，合计 96 对象 / 86 关系 / 78 Evidence；没有为新增锚点编造影响边。页面公开地区、品类与近年覆盖缺口。
...
- 地图模式在选中品类后提供两种可逆视角：代表作品视角保留完整 69 节点／55 关系网络，品类发展视角同样保留全图，只把匹配的 Innovation Event 与演进关系提高强调；游戏、硬件与实验程序不会被误当作消失的数据。
- Innovation Atlas 现在把“创新事件路线 → 承载作品”作为品类发展视角的阅读顺序：事件节点覆盖第一人称射击视角、垂直空间战斗、网络化战斗空间、角色成长、开放世界非线性探索、锁定目标战斗与程序生成单局结构；点击事件仍可进入 Doom、Quake、Half-Life 等承载证据。选择 Genre Family 后只改变强调和路线语义，不把任一品类隔离成独立网络；无证据的品类继续显示明确空状态。
- 品类发展视角现在把 Innovation Event 放在地图的中央主带：事件按时间形成可读的演进层，承载作品与硬件／实验程序下移为证据层；代表作品视角仍保留原有作品优先的同一张全图，两个视角不复制、不删除实体。
- Innovation Atlas 进一步把事件节点放回地图主叙事：移除地图前的 7 张平铺事件卡，保留默认收起的紧凑索引；事件节点使用强调边框与更高层级，事件演进关系使用连续强调线，承载作品保持次级虚线样式。
- 资源目录当前已有 3120 个 Work Item；此前已完成的英文优先 GDC Vault 会话、中文与少量日文条目继续保留。本轮新增 1000 条 Game Developer 官方文章，没有重复导入 URL；后续扩展仍以逐页核验、规范化去重和主题/能力错配审计为门槛。
- 资源表事实筛选改为常驻紧凑工具栏：七项筛选、搜索、结果数和展开／收起在同一表头直接可见，不再额外包一层“按事实筛选”标签；访问版本详情提高层级并使用明确不透明背景，避免透出相邻 Work Item。
- 通过全站短文案换行检查，首页、EGDS、Career、Atlas 与资源页的说明文本使用 pretty 换行与中文严格断行，减少被截断的孤字与只有一两个字的第二行；EGDS 长标题和框架节点标签也不再被窄列强制截断。
- 新增 1000 条经 Game Developer 官方页面 HTTP 200 与页面 metadata 核验的英文 Work Item；目录达到 41 个 Source、3120 个 Work Item 与 3134 个 Access Version。此前 GDC 会话继续统一保守标记为 subscription，不把搜索摘要或录播可用性推断为事实。
- 减少 Career、EGDS 与资源页重复装饰分割线，保留表格行和必要结构边界；资源表标题、搜索、事实筛选、结果数与展开操作继续共用紧凑表头。
