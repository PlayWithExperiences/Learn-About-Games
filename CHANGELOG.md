...
- 地图模式在选中品类后提供两种可逆视角：代表作品视角保留完整 66 节点／50 关系网络，品类发展视角同样保留全图，只把匹配的 Innovation Event 与演进关系提高强调；游戏、硬件与实验程序不会被误当作消失的数据。
...
- 资源目录当前已有 2120 个 Work Item；此前已完成的英文优先批次（1000 条 GDC Vault 会话及 10 条 Game Developer 复盘）继续保留。本轮没有重复导入同一批 URL，后续扩展仍以逐页核验、规范化去重和主题/能力错配审计为门槛。
...
- 新增 1000 条经 GDC Vault 官方 sitemap 与会话页 HTTP／metadata 核验的英文 Work Item，并补入 10 条经 Game Developer 官方页面核验的设计复盘；目录达到 41 个 Source、2120 个 Work Item 与 2134 个 Access Version。GDC 会话统一保守标记为 subscription，不把搜索摘要或录播可用性推断为事实。
