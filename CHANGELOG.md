...
- 地图模式在选中品类后进入可逆的事件路线：主画布只显示匹配的 Innovation Event 与演进关系，游戏、硬件与实验程序保留为事件详情中的承载证据；切回“全部网络”恢复完整网络。
- Innovation Atlas 改为“创新事件路线 → 承载作品”的阅读入口：当前 7 个事件节点覆盖第一人称射击视角、垂直空间战斗、网络化战斗空间、角色成长、开放世界非线性探索、锁定目标战斗与程序生成单局结构；全网为 66 个节点、50 条关系、73 项 Evidence。选择 Genre Family 后主区只显示有证据的事件路线，游戏在详情中作为 carrier closure，不宣称唯一发明者；无证据的品类显示明确空状态。
