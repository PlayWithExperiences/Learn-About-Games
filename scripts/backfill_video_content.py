...
def process(args: argparse.Namespace) -> int:
    ...
    selected = select_candidates(resources, state, args.limit, args.retryable, candidate_ids)
    report: dict[str, Any] = {
        "startedAt": now_iso(),
    ...
    for item in selected:
        ...
        try:
            ...
            update_record(state, item_id, {"status": "pending_write", "result": result, "model": model_result["model"]}, args.state)
            item.clear()
            item.update(updated)
            atomic_write_json(args.resources, resources)
            update_record(state, item_id, {"status": "completed", "model": model_result["model"], "summaryLength": len(result["summary"]["zh-CN"])}, args.state)
            consecutive_channel_failures = 0
            ...
            if isinstance(exc, ModelChannelError):
                ...
                status = "retryable"
            update_record(state, item_id, {"status": status, "failureClass": failure_class, "retryable": status == "retryable"}, args.state)
