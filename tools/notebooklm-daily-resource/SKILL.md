...
- Before the first claim, run a non-consuming NotebookLM workbench readiness check. The active page must be an editable `notebook.google.com/notebook/...` page with the source panel and Studio controls available. If it redirects to `/accessrequest`, `?location=unsupported`, or any read-only/unsupported page, stop the run before claiming a candidate and report `browser-notebook-access`; do not spend a claim on a workbench that cannot accept a source.
...
3. In the user's already-authenticated NotebookLM browser session, reuse the persistent Learn About Games work Notebook when available; create one only if no work Notebook can be found and the readiness check confirms that creation is possible. Import the selected public YouTube URL as a new source, then select only that source in the source panel and every Studio generator. Use NotebookLM's actual `notebook.google.com` page, not a Gemini wrapper. Generate the result using the fixed contract in [references/resource-contract.md](references/resource-contract.md).
...
5. Export or download the infographic, fully expanded mind map, and detailed Chinese slides. For the mind map, open the generated card in NotebookLM's map viewer before downloading; use `更多选项` → `全部展开`, wait for rendering to settle, and visually verify at least three node levels and zero remaining `>`/collapsed-node indicators. Record this observation in the temporary result JSON exactly as `expansion_verification: {"method":"notebooklm-viewer","action":"全部展开","observed_depth":<integer >= 3>,"collapsed_node_count":0}`. A default card PNG without this viewer action is not a valid mind map. If the control is unavailable, rendering never settles, or any collapsed indicator remains, fail at phase `mind-map-expansion` and do not publish a ready record. Arm the browser download listener before clicking the NotebookLM download control, save the actual download to a fresh temporary path, and validate non-zero bytes, expected file type/content signature, and dimensions where applicable. A click, a page asset listing, a viewer URL, or a successful HTTP response alone is not an export. If the first export control does not emit a validated download, use the artifact's visible NotebookLM viewer/download path once; do not fabricate a screenshot or extract bytes from an unusable page context. Keep the original deck when NotebookLM exposes it; otherwise keep the generated deck. A PDF may be retained as a regular link, not an embedded image.
...
8. Publish atomically through the producer script, which adds the stable `resource_id`, `source.video_id`, `contract_version: 2`, `producer_status: "ready"`, `generation_run_id`, `generated_at`, and `output_fingerprint`. The producer rejects a mind map without the viewer expansion verification, so do not bypass or soften that error:
...
9. If a candidate fails at any phase, run the terminal `fail` command with the exact phase and continue to the next candidate already returned by preflight. Use `mind-map-expansion` when the viewer did not produce a demonstrably fully expanded map. Do not retry the failed candidate, and do not let one failure erase the other claims or hide the failed ledger entry.
