...
## 补充：把等待移出 goal 轮次（2026-09-13 01:40 +0800）

- goal 轮次间隔约 1–2 分钟，而闸门在约 4:00 之后才开——若靠轮次轮询，会白跑近百轮。
- 因此新增 `automation/wait-for-deck-gate.sh`：每 5 分钟探测一次演示文稿可用性，最多等 4 小时；
  已作为**后台作业**启动（`bash-10`），闸门一开即结束并通知，不再占用 goal 轮次。
  退出码：0=闸门已开，3=到时限仍关闭，1=探测连续失败（浏览器通道问题）。
- **闸门开启后的收尾顺序**（留给下一轮，含验证）：
  1. 读 Studio 卡片，确认 item 3 的新演示文稿标题（当前占位文案是「已安排在 4上午之后生成」）；
  2. `retry youtube-gPV0qmyGs-o --previous-generation-run-id run-20260913010350-19594`（消耗 1 次 claim）；
  3. `finish-notebooklm-item.py --infographic 叙事驱动游戏留存的艺术 --mindmap 叙事设计 思维导图 --slides <新标题>`
     —— 只补导出/上传/发布/交付，不重做已生成的图像类产物；
