...
- 决策：無涘 ｜ 记录：AI。发现官方 YouTube 描述也是可追溯的正文入口：目标中约 2,096 条描述去除 URL 后至少 240 个字符。新增 `description` 输入模式；元数据标记无字幕且描述达标时优先使用描述，避免额外触发 YouTube 通道，描述不足才转音频。首批 76 条描述生产回写完成，当前总完成数 127、剩余 2,137 条。
- 时间估算：AI 推断。最近 50 条描述批次约 10 分钟完成且无新增失败；描述路线成功率与模型吞吐仍需扩大样本后再估算，不能用这 50 条替代剩余 2,137 条的完成证据。
