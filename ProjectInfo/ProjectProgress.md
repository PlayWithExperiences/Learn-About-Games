...
- YouTube 内容补全当前累计 427 completed、4 no_transcript、2 transcript_insufficient、41 channel_failure，生产目录剩余 1,837 条；其中 376 条以 `inputMode=description`、1 条以 `transcript`、1 条以 `audio` 完成，另有 49 条历史回写没有入口字段。最近两轮 100 条官方描述批次均 100/100 成功，无新增模型或通道失败；原始音频、字幕、官方元数据和状态仍在仓库外缓存。缓存审计显示目标中约 2,096 条描述达到 240 字符门槛，公开频道 RSS 对 GMTK 返回 404。
...
- 时间估算（AI 推断）：描述路线不受 YouTube 请求冷却影响；最近 50 条约 10 分钟，第二轮 100 条约 26.5 分钟，第三轮 100 条约 20.5 分钟且均 100/100 成功。不能把该样本直接外推为全量固定日期，因为尚有描述不足、字幕通道失败和音频转写条目。涓流现已改为每 4 小时优先批量处理最多 20 条描述；若剩余 1,837 条都能走描述路线，理论约 15–16 天；其余条目仍受 YouTube 约 6 条／天的安全速率约束。
