...
- YouTube 内容补全当前累计 51 completed、4 no_transcript、2 transcript_insufficient、41 channel_failure，生产目录剩余 2,213 条；本轮把两个只有 `[Music]` 音频标记的缓存从 model_failure 重新归为 `transcript_insufficient`，并验证了两条生产入口：字幕条目 `awegilW3DTc` 已写回，字幕不可分析的 `chWr87u3Gdc` 通过“yt-dlp 下载 MP3 → Vertex/Gemini ADC → 字段校验 → pending_write → 原子回写”完成，状态记录明确为 `inputMode=audio`。原始音频、字幕和状态仍在仓库外缓存。现有 priority 涓流约每 4 小时尝试 1 条；目标中仍有 239 条标记有字幕且未完成，按约 6 条／天粗估约 40 天，其余约 2,021 条尚未完成，需要按音频路线继续处理。公开频道 RSS 对 GMTK 返回 404。
...
- 时间估算（AI 推断）：当前安全涓流为约 6 条／天，239 条字幕路线约 40 天；若剩余 2,213 条都按同一频率且没有额外冷却，理论上约 369 天（约 12 个月），但成功率和 YouTube 通道状态会改变结果，不能承诺固定日期。Vertex 音频路线已验证可行，但只验证了单条，尚未获得足够样本估计“无字幕”批次的成功率和成本；先按逐条可观察运行，达到一小批后再更新估算。
