...
- YouTube 内容补全当前累计 1,527 completed、4 no_transcript、2 transcript_insufficient、41 channel_failure；真正未完成 784 条，其中 737 条尚未进入已分类状态，47 条已经尝试但仍是显式非完成状态。完成入口为 1,476 条 `inputMode=description`、1 条 `transcript`、1 条 `audio`，另有 49 条历史回写没有入口字段。最近十四轮 100 条官方描述批次最终均 100/100 完成；第十四轮无新增模型或通道失败，当前 model failure 为 0。原始音频、字幕、官方元数据和状态仍在仓库外缓存。缓存审计显示目标中约 2,096 条描述达到 240 字符门槛，公开频道 RSS 对 GMTK 返回 404。
...
- 时间估算（AI 推断）：描述路线不受 YouTube 请求冷却影响；最近十四轮 100 条约 17.6–28.1 分钟，最终均 100/100 成功。不能把该样本直接外推为全量固定日期，因为尚有描述不足、字幕通道失败和音频转写条目。涓流现已改为每 4 小时优先批量处理最多 20 条描述；若当前 737 条尚未分类条目都能走描述路线，理论约 6–7 天；另外 47 条显式非完成条目仍受字幕／音频通道和冷却约束，不能用描述吞吐估算。
