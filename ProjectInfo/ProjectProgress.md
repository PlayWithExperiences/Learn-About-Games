...
- YouTube 内容补全当前累计 127 completed、4 no_transcript、2 transcript_insufficient、41 channel_failure，生产目录剩余 2,137 条；本轮新增并验证第三条正文入口：官方 YouTube 描述去除 URL 后达到 240 字符以上时，直接作为独立证据文本交给模型分析，已有 76 条以 `inputMode=description` 写回；字幕条目 `awegilW3DTc` 与音频条目 `chWr87u3Gdc` 仍分别保留 `transcript`/`audio` 来源。原始音频、字幕、官方元数据和状态仍在仓库外缓存。现有 priority 涓流约每 4 小时尝试 1 条；缓存审计显示目标中约 2,096 条描述达到该门槛，实际描述批次已先处理 76 条。公开频道 RSS 对 GMTK 返回 404。
...
- 时间估算（AI 推断）：描述路线不受 YouTube 请求冷却影响，最近 50 条顺序批次约 10 分钟完成，模型通道稳定；但不能把这个模型吞吐直接外推为全量固定日期，因为尚有描述不足、字幕通道失败和音频转写条目。当前安全 YouTube 涓流仍约 6 条／天；剩余 2,137 条若全部只能按该速率成功，理论约 356 天。随着描述批次扩大，将按实际成功率重新估算，而不是沿用纯音频的保守上限。
