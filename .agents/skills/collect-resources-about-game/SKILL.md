...
导出时先监听真实下载事件，再点击 NotebookLM 可见的下载控件；每个文件写入全新临时路径并验证非零字节、文件签名/类型及适用时的图片尺寸。若首条可见下载路径未产生可验证文件，同一候选只允许再试一次明确的 viewer/download 路径；不要读取不可用页面上下文、伪造截图或把 URL 当文件。Slides 允许较长等待（当前合同单条最多 15 分钟），不能在短等待后把仍在生成的状态判为成功或失败。
