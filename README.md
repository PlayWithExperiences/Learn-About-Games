...
- Innovation Atlas 共用一份 195 节点、160 条有证据关系与 198 项 Evidence 的 catalog。`/atlas/` 以年代分列、十三个阅读区域显示作品、创新观察与类别形成；点选后追踪真实相邻关系并打开来源。七个问题入口用于比较阅读，不生成影响边。创新观察的日期指向讨论实例，不代表最早发明。`/atlas/network/` 保留精确年份坐标、缩放与平移，以及 10 个常见 Genre Family 的非排他浏览入口。两个视图共享事实，搜索和选择保留总览位置；无 JavaScript 时可通过年代目录查阅关系与来源。此为选择性历史，地区、品类与近年覆盖缺口在页面明确标示。
...
覆盖盘点：`node scripts/audit-atlas-coverage.mjs`，直接复用地图的阅读区分配；输出不代表完整游戏史覆盖率。

2026-09-13已逐项复核现有Atlas对象、关系与原证据；新增材料、勘误和未确认的直接影响见[全量核查报告](docs/research/2026-09-13-atlas-full-verification.md)。来源支持程度与结构覆盖分开，不以连续箭头代替因果证据。
