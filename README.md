...
- 成长资源收录 43 个 Source、5437 个具体 Work Item、5590 个 Access Version 与 16 个无顺序资源主题。主目录默认折叠为 16 个可独立展开的主题子表，也可展开全表；搜索框与七项事实筛选直接位于资源表头，筛选不再包在额外的“按事实筛选”折叠框里。Source 目录拥有独立子入口。本轮完整收录 GDC Festival of Gaming、Game Maker's Toolkit 与 Masahiro Sakurai on Creating Games 英文频道的公开视频；每条保留官方播放页、免费访问事实和一个保守主题，不为凑数量填充 `whyRelevant` 或未经标题支持的细分类。此前英文 GDC 会话、中文与少量日文核验条目继续保留；GDC Vault 会话仍保守标记为 subscription。
