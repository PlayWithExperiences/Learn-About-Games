...
- Innovation Atlas 共用一份 96 节点、86 条有证据关系与 78 项 Evidence 的 catalog。`/atlas/` 以年代分列、九个阅读区域显示作品、创新观察与类别形成；点选后追踪真实相邻关系并打开来源。三个问题入口用于比较阅读，不生成影响边。创新观察的日期指向讨论实例，不代表最早发明。`/atlas/network/` 保留精确年份坐标、缩放与平移，以及 10 个常见 Genre Family 的非排他浏览入口。两个视图共享事实，搜索和选择保留总览位置；无 JavaScript 时可通过年代目录查阅关系与来源。此为选择性历史，地区、品类与近年覆盖缺口在页面明确标示。
