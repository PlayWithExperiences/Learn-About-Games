...
- 成长资源收录 29 个 Source、148 个具体 Work Item 与 15 个无顺序资源主题。主目录默认折叠为 15 个可独立展开的主题子表，也可展开全表后使用同一组表头事实筛选；Source 目录拥有独立子入口。
- Innovation Atlas 是一张 36 节点、30 条有证据关系与 49 项文献构成的全局时间网络。桌面只有明确进入“地图模式”后，普通滚轮才围绕指针连续缩放并允许拖动；模式外滚轮仍滚动页面。早期切片从 1958 年延伸到 Pong，但不把 Pong 写成未经限定的“第一款电子游戏”。
