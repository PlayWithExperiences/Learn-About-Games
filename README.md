...
- 成长资源收录 41 个 Source、1110 个具体 Work Item、1124 个 Access Version 与 15 个无顺序资源主题。主目录默认折叠为 15 个可独立展开的主题子表，也可展开全表后用搜索框与同一组表头事实筛选；Source 目录拥有独立子入口。本轮新增 500 条英文 GDC Vault 官方会话：既有英文、中文与少量日文核验条目继续保留。每条仍逐页核验、规范化去重、恰属一个主题，不为凑数量收录证据不足的候选；GDC 会话保守标记为 subscription。
