...
- 成长资源收录 43 个 Source、5430 个具体 Work Item、5583 个 Access Version 与 16 个无顺序资源主题。主目录默认折叠为 16 个可独立展开的主题子表，也可展开全表；搜索框与七项事实筛选直接位于资源表头，筛选不再包在额外的“按事实筛选”折叠框里。Source 目录拥有独立子入口。本轮完整收录 GDC Festival of Gaming、Game Maker's Toolkit 与 Masahiro Sakurai on Creating Games 英文频道的公开视频；每条保留官方播放页、免费访问事实和一个保守主题，不为凑数量填充 `whyRelevant` 或未经标题支持的细分类。此前英文 GDC 会话、中文与少量日文核验条目继续保留；GDC Vault 会话仍保守标记为 subscription。
- Innovation Atlas 是一张 84 节点、86 条有证据关系与 76 项 Evidence 构成的全局时间网络。10 个常见 Genre Family 只负责非排他的浏览入口；网络把“创新事件”和“承载作品”分开标记：事件是时间线的主叙事，作品是事件详情中的承载证据。第一人称射击视角、锁定目标的空间战斗、即时战略的资源生产与非对称阵营等事件，会在同一张全局网络中互相影响；选择品类只改变高亮，不切断跨品类关系。事件卡先解释发生了什么、何时出现、如何被引入和扩散，再列出支持这些判断的作品与证据，不宣称唯一发明者；Genre 谱系仍只是可核查的选择性透镜，不冒充完整品类史。桌面只有明确进入全屏“地图模式”后，普通滚轮才围绕指针连续缩放并允许拖动；“适应全图”会以完整内容为先，必要时可以低于手动缩放的 50% 下限。
