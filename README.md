...
- 能力地图包含 28 个 EGDS 方法节点、42 个可实践能力、12 个知识议题和 64 条 `supports` / `complements` 关系。它以 PlayWithExperiences 的 EGDS 为作者化骨架：从体验旅程与情绪曲线出发，从感受、理解、解构走向重构，再进入叙事、美学与表现、玩法与挑战等设计杠杆；生产、团队、产品与更广语境形成其余四条条件分支。桌面先显示一屏方法骨架、一次只展开一个叶容器；中间宽度、移动端与无 JavaScript 使用同源原生大纲。
...
- 成长资源收录 38 个 Source、174 个具体 Work Item 与 15 个无顺序资源主题。主目录默认折叠为 15 个可独立展开的主题子表，也可展开全表后使用同一组表头事实筛选；Source 目录拥有独立子入口。
- Innovation Atlas 是一张 36 节点、30 条有证据关系与 49 项文献构成的全局时间网络。桌面只有明确进入“地图模式”后，普通滚轮才围绕指针连续缩放并允许拖动；模式内仍可切换 Roguelike 与 Metroidvania 证据透镜。早期切片从 1958 年延伸到 Pong，但不把 Pong 写成未经限定的“第一款电子游戏”。
